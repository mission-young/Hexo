...
![](images/LeetCode破局攻略/kmp/txt1.jpg)

`dp` 数组指示 `pat` 这样移动：

![](images/LeetCode破局攻略/kmp/txt2.jpg)

PS：这个`j` 不要理解为索引，它的含义更准确地说应该是**状态**（state），所以它会出现这个奇怪的位置，后文会详述。

而对于 `txt2` 的下面这个即将出现的未匹配情况：

![](images/LeetCode破局攻略/kmp/txt3.jpg)

`dp` 数组指示 `pat` 这样移动：

![](images/LeetCode破局攻略/kmp/txt4.jpg)
...
![](images/LeetCode破局攻略/kmp/kmp.gif)
...
![](images/LeetCode破局攻略/kmp/shadow1.jpg)
...
![](images/LeetCode破局攻略/kmp/A.gif)

当然，如果遇到的字符是 "B"，状态 `X` 也不能进行「状态推进」，只能回退，`j` 只要跟着 `X` 指引的方向回退就行了：

![](images/LeetCode破局攻略/kmp/shadow2.jpg)
...
![](images/LeetCode破局攻略/kmp/dfa.gif)
...
[上一篇：贪心算法之区间调度问题](/2016/01/01/动态规划系列/贪心算法之区间调度问题)

[下一篇：团灭 LeetCode 股票买卖问题](/2016/01/01/动态规划系列/团灭股票问题)
